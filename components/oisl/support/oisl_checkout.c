...
** Include Files
*/
#include "oisl_checkout.h"


/*
** Global Variables
*/
uart_info_t OislUart;
OISL_Device_HK_tlm_t OislHK;
OISL_Device_Data_tlm_t OislData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = OISL_CommandDevice(&OislUart, OISL_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = OISL_RequestHK(&OislUart, &OislHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("OISL_RequestHK command success\n");
                }
                else
                {
                    OS_printf("OISL_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_OISL:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = OISL_RequestData(&OislUart, &OislData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("OISL_RequestData command success\n");
                }
                else
                {
                    OS_printf("OISL_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = OISL_CommandDevice(&OislUart, OISL_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    OislUart.deviceString = OISL_CFG_STRING;
    OislUart.handle = OISL_CFG_HANDLE;
    OislUart.isOpen = PORT_CLOSED;
    OislUart.baud = OISL_CFG_BAUDRATE_HZ;
    status = uart_init_port(&OislUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", OislUart.deviceString, OislUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", OislUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&OislUart);

    OS_printf("Cleanly exiting oisl application...\n\n"); 
    return 1;
}
