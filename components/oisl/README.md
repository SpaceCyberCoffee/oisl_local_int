# Oisl - NOS3 Component
This repository contains the NOS3 Oisl Component.
This includes flight software (FSW), ground software (GSW), simulation, and support directories.

## Component Template
This component template utilizes canned files from the existing [oisl component](https://github.com/nasa-itc/oisl) to generate a new component to do development in.
...
## Overview
In addition to being used by the template generator, the oisl component provides an executable example for the user.  This oisl component is a UART device that accepts multiple commands, including requests for telemetry and data.
The available FSW is for use in the core Flight System (cFS) while the GSW supports COSMOS.
A NOS3 simulation is available which includes both oisl and 42 data providers.
...
## FSW
Refer to the file [fsw/platform_inc/oisl_platform_cfg.h](fsw/platform_inc/oisl_platform_cfg.h) for the default
...
<simulator>
    <name>oisl_sim</name>
    <active>true</active>
    <library>liboisl_sim.so</library>
    <hardware-model>
        <type>OISL</type>
        <connections>
            <connection><type>command</type>
                <bus-name>command</bus-name>
                <node-name>oisl-sim-command-node</node-name>
            </connection>
...
* make
* make checkout
  * Launches NOS Engine, NOS Time Driver, NOS Terminal, Oisl Sim, and Oisl Checkout
* make stop

# Documentation
If this oisl application had an ICD and/or test procedure, they would be linked here.
